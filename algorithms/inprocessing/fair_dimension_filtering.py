...
from MAF2024.datamodule.dataset import CelebADataset, aifData
...
class DimFiltering:
    def __init__(
        ...
        self.device = device
        self.model_dir = os.environ["PYTHONPATH"] + "/MAF2024/model/FairFiltering/"
    ...
    def train(self, models_):
        ...
        for ep in range(self.n_epoch):
            ...
            for idx, data in enumerate(pbar):
                ...
                class_losses = [loss[target == i].mean() for i in range(self.n_class)]
                class_losses = torch.stack(class_losses)

                diff = torch.nan_to_num(
                    torch.abs(class_losses[0] - class_losses[1]), 0.0
                )  # max - min
                if isinstance(models_, Filter_Net):
                    loss_for_update = loss.mean() + diff
                else:
